...
def procesar_datos(df):
    ...
    columnas_fecha = [
        ...
        'fecha_solicitud_a_proveedor', 'fechas_solicitud_almacen'
    ]
    ...
    if 'motivo_del_estado' in df.columns:
        df['motivo_del_estado'] = df['motivo_del_estado'].str.strip().str.upper()

    # --- Reglas de Filtrado con validación de columnas ---
    if 'tipo_de_solicitud' in df.columns:
        ...
        condicion_tipo_solicitud = pd.Series([False] * len(df), index=df.index)
        condicion_tipo_spms = pd.Series([False] * len(df), index=df.index)

    if 'estado' in df.columns:
        condicion_estado = df['estado'].isin(['CLOSED COMPLETE','CERRADO','CERRADO INCOMPLETO'])
    else:
        condicion_estado = pd.Series([False] * len(df), index=df.index)

    if 'motivo_del_estado' in df.columns:
        condicion_motivo = df['motivo_del_estado'] == 'ATENCIÓN COMPLETADA'
    else:
        condicion_motivo = pd.Series([False] * len(df), index=df.index)
    ...
    # --- Filtro Prioridad con validación ---
    if 'prioridad' in df.columns:
        ...
        filtro_prioridad_baja_media = pd.Series([False] * len(df), index=df.index)
        filtro_prioridad_alta_critica = pd.Series([False] * len(df), index=df.index)
    ...
    if 'fecha_de_creacion' in df.columns and 'fechas_solicitud_almacen' in df.columns:
        tiempo_transcurrido_1 = abs(df['fecha_de_creacion'] - df['fechas_solicitud_almacen'])
   ...
   ###########################################  ETAPA 1 'SPMS,SPMS + ATT' ###################################################
    ...
    # --- Calcula la diferencia para Tiempo 1 ---
    if 'fecha_de_creacion' in df.columns and 'fecha_solicitud_a_proveedor' in df.columns:
    ...
    if 'fechas_solicitud_almacen' in df.columns and 'fecha_liberacion_almacen' in df.columns:
        tiempo_transcurrido_2 = abs(df['fechas_solicitud_almacen'] - df['fecha_liberacion_almacen'])
    ...
    # --- calcular la diferencia parta Timepo 3 ---
    if 'fecha_liberacion_almacen' in df.columns and 'fecha_entrega_en_sitio' in df.columns:
